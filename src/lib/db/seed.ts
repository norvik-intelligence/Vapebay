/**
 * Seeds the database from the in-repo catalog.
 *
 * Run with `npm run db:seed` (after `npm run db:push` has created the tables).
 * Idempotent: every insert is an upsert keyed on the primary key, so re-running
 * refreshes prices and stock without duplicating rows.
 */
...
function seed() {
  ...
  // One transaction for the whole catalog: 250 individual commits would each
  // fsync the WAL, turning a sub-second seed into ~20 seconds on a VPS disk.
  db.transaction((tx) => {
  ...
  console.log('→ Beispiel-Altersnachweise');
  const sample = [
  ...
  for (const [index, entry] of sample.entries()) {
    db.insert(ageVerifications)
      .values({
        ...entry,
        checkedAt: new Date(Date.now() - index * 3_600_000).toISOString(),
      })
      .onConflictDoNothing()
      .run();
  }
  ...
  console.log(
    `\n✓ Seed abgeschlossen: ${BRANDS.length} Marken, ${DEVICES.length} Geräte, ${FLAVOURS.length} Profile, ${PRODUCTS.length} Produkte, ${SUPPLIERS.length} Lieferanten.`,
  );
}
